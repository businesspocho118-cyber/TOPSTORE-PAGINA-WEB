  ...
  return (
    <footer className="border-t border-ink/10 bg-ink text-white">
      <div className="container-luxe grid gap-10 py-14 md:grid-cols-[1.2fr_1fr_1fr] md:py-20">
        ...
        <div>
          <p className="eyebrow text-champagne">Redes</p>
          <div className="mt-5 grid gap-3 text-sm text-white/74">
            {socialLinks.map(({ href, label, handle, icon }) => (
              <Link key={label} href={href} target="_blank" rel="noreferrer" className="flex min-h-11 items-center gap-3 transition hover:text-champagne">
                {icon}
                <span>{handle}</span>
              </Link>
            ))}
      ...
      <div className="border-t border-white/10 py-6 text-center text-xs uppercase tracking-[0.2em] text-white/45">
        © 2026 TOPSTORE. Todos los derechos reservados.
      </div>
    </footer>
  )
}
